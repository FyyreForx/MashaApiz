...
let apkdl = {
  search: async function(args) {
    let res = await fetch(tools.Proxy(tools.api(2, '/search', { q: args })));
    res = await res.text();
    let $ = cheerio.load(res);
    let link = [];
    let name = [];
    ...
    $('a.title').each(function(a, b) {
      let nem = $(b).text().trim();  // Trim whitespace
      nem = nem.replace(/<\/?[^>]+(>|$)/g, ""); // Remove any remaining HTML tags
      name.push(nem);
    ...
    for (let i = 0; i < name.length && i < link.length; i++) {
      ress.push({
        name: name[i],
        link: link[i]
      });
...
module.exports = { aptoide, apkcombo, apkdl }